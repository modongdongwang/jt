...
public class ItemServiceImpl implements ItemService {
	...
	public void saveItem(Item item,ItemDesc itemDesc) {
		...
		//mybatis中如果设定主键自增,则新增时Id会自动回调
		//insert into xxx values(xxx)
		//sql:SELECT LAST_INSERT_ID();之后将数据交给spring容器
	 ...
	 * SUPPORTS 事务支持,查询之前有事务的则和并事务
	 * 
	 * rollbackFor="包名.类名.名称.class" "异常类型",回滚事务
	 * norollbackFor="异常类型"不回滚
	 * readOnly=true 不允许修改数据,只读
	 * 
	 * String中默认事务控制策略:
	 * 1.检查异常/编辑异常 不负责事务控制
	 * 2.运行时异常/error 回滚事务
